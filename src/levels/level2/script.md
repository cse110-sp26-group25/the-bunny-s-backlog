...
Bill the Lizard: "Alright, the console found the directory. I usually just fix cracked wall screens, but sure, let's hunt down a fugitive."
...
Look at the active file structures pinned to your workspace:

const suspect1 = { earShape: "small", height: "small", sound: "hoot" }
const suspect2 = { earShape: "big", height: "small", sound: "bark" }
const suspect3 = { earShape: "big", height: "small", sound: "squeak" }"

### Player Input

(None. This phase transitions after clicking to close the preview window).
...
```bash
🟢 Objects loaded: suspect1, suspect2, suspect3 ready for verification.
```
...
"After we try something, we use catch to keep track of the contradictions (errors) that occurred. We specify which particular errors we're looking for inside catch(errorType) and how we should respond to that error with { responseToError }. For example: catch(moldyCarrot) { self.throwAway(carrot); }
...
Police Officer Dog: "These files contain information regarding our line-up. We're looking for someone who matches the exact profile our bat friend gave us. If anyone doesn’t match the description, it would be a contradiction (error) to keep them here! For every suspect, let's look out for the explicit mismatches: notBigEars, notSmall, and doesNotSqueak. If one pops up, we'll increment our counter variable."
...
Bill the Lizard: "Running the cross-reference check now... Let's see if our data matches the bat's late-night memory."
...
Police Officer Dog: "Look at that! The owl triggered 2 contradictions (errors) because it has small ears and doesn't squeak, and the dog hit 1 contradiction (error) because it doesn't squeak either. But the mouse profile cleared with zero contradictions! Run your console string using backticks so we can lock this down."
