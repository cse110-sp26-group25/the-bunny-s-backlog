...
const StarField = (() => {
  ...
  const CONFIG = {
    ...
    LAYER_RATIOS: [.2, .4, .4], // near : mid : far distribution
    LAYER_SIZES:  [2.2, 1.4, .7],
    LAYER_ALPHA:  [.95, .65, .35],
    TWINKLE_SPEED: .012,        // base twinkle phase increment per frame
  };
  ...
  function render() {
    ...
    for (const star of stars) {
      ...
      star.alpha = star.baseAlpha * (.6 + .4 * Math.sin(star.phase));
...
const CreditsCrawl = (() => {
  ...
  const BASE_DURATION = parseFloat(
    getComputedStyle(document.documentElement)
      .getPropertyValue('--crawl-duration')
  );
  ...
  function slower() {
    ...
    pxPerSecond = Math.max(pxPerSecond * 0.7, (startY - endY) / (BASE_DURATION * 3));
  }

  function faster() {
    // Increase speed by ~43% — same relative step as before
    pxPerSecond = Math.min(pxPerSecond * 1.4, (startY - endY) / (BASE_DURATION * 0.25));
  }
