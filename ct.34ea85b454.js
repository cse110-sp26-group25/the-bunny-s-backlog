...
${ev.error?.stack ?? "(no stack available)"}`;
  ...
  var meta = [
    {"name":"typing-game","author":"","site":"","version":"0.0.0"}
  ][0];
...
styles.new(
    "TerminalStyle",
    {
    "fontFamily": "JetBrainsMono-VariableFont_wght, sans-serif",
    "fontSize": 18,
    "fontStyle": "normal",
    "fontWeight": "400",
    "align": "left",
...
templates.templates["TerminalBox"] = {
    name: "TerminalBox",
    depth: 0,
    ...
    onStep: function () {
        /* 🐱👉 template TerminalBox — On frame start event (core_OnStep) */
{
let isBlinking = Math.floor(Date.now() / 500) % 2 === 0;

let cursor = isBlinking ? "_" : " ";

this.myText.text = ">   " + window.gameState.currentInput + cursor;
}
    ...
    onCreate: function () {
        /* 🐱👉 template TerminalBox — On create event (core_OnCreate) */
{
this.x = camera.width / 2;
this.y = camera.height / 2;

let boxWidth = camera.width * 0.5;
let boxHeight = 65;

this.bgShape = new PIXI.Graphics();
this.bgShape.beginFill(0x22223b, 0.95);
this.bgShape.drawRoundedRect(-boxWidth / 2, -boxHeight / 2, boxWidth, boxHeight, 15);
this.bgShape.endFill();

this.addChild(this.bgShape); 

this.myText = new PIXI.Text("", styles.get('TerminalStyle'));

this.myText.anchor.set(0, 0.5);
this.myText.x = -boxWidth / 2 + 20;
...
templates.list['TerminalBox'] = [];
        
    
    
rooms.templates['main'] = {
    name: 'main',
    width: 1280,
    height: 720,
    behaviors: JSON.parse('[]'),
    objects: JSON.parse('[{"x":512,"y":256,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"TerminalBox"},{"x":640,"y":-192,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"TerminalBox"},{"x":640,"y":-192,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"TerminalBox"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[]'),
    ...
    onLeave() {
        /* 🐱👉 room main — On room end event (core_OnRoomEnd) */
{
window.removeEventListener('keydown', this.typingListener);
    ...
    onCreate() {
        /* 🐱👉 room main — On room start event (core_OnRoomStart) */
{
